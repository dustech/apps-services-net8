...
using Npgsql; // To use NpgsqlConnection
...
internal partial class Program
{
    ...
    private static string ReadPassword()
    {
        ...
        while (true)
        {
            ConsoleKeyInfo
                info = Console
                    .ReadKey(true); // true per non visualizzare il carattere
            ...
            if (info.Key == ConsoleKey.Backspace)
            {
                if (password.Length > 0)
                {
                    ...
                    Console.Write(
                        "\b \b"); // Simula la cancellazione del carattere dalla console
                }
            }
            else
            {
                ...
                Console.Write(
                    "*"); // Visualizza un asterisco per ogni carattere digitato
            }
        }
